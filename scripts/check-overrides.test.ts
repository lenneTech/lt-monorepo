...
/**
 * A minimal `expect` mapped onto node:assert, so this file can stay byte-identical to
 * nest-server's vitest copy apart from these few lines.
 *
 * The alternative — rewriting every assertion — was tried three times and failed three
 * different ways: two stray `expect(` residues, two hybrids like
 * `assert.match(x).toMatch(re)`, and once a `.not.toMatch` whose NEGATION was dropped,
 * which would have inverted the test while still reading as a real assertion. The
 * message parameter sits at position 2 in vitest and last in assert, so any pattern
 * that misses a shape leaves either a residue or a silent inversion.
 *
 * A shim removes the rewrite entirely: the two runners now differ by an import block,
 * and a future sync is a copy plus these lines.
 */
const expect = (actual: unknown, message?: string) => ({
  not: {
    toBe: (expected: unknown) => assert.notEqual(actual, expected, message),
    toMatch: (re: RegExp) => assert.doesNotMatch(String(actual), re, message),
  },
  ...
  toMatch: (re: RegExp) => assert.match(String(actual), re, message),
});
const afterAll = after;

const GUARD = join(import.meta.dirname, 'check-overrides.mjs');
