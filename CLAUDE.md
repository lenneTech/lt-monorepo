...
- **CI** — GitLab (`.gitlab-ci.yml`) and GitHub Actions (`.github/workflows/test.yml`) both: build once (`build` job → `projects/api/dist` + `projects/app/.output` as an artifact), then run **two shards** that boot the **compiled** API (`start:e2e:dist`, migrations first) on `:3000`, set `PLAYWRIGHT=true` and `E2E_BUILT_SERVER=true`, and let Playwright serve the **built** Nuxt server on `:3001`. The shard reports are merged into one HTML report by a follow-up job.
  ...
  It further asserts that **every script a pipeline calls is one the target package actually defines** — `start:e2e:dist` was once referenced by both pipelines while no package defined it, and the ordering rule above happily confirmed that a nonexistent script ran after `migrate:up`. The target is resolved from a `cd`, from `-C`/`--dir`, or from `pnpm --filter=<package-name>` looked up through the `packages:` globs in `pnpm-workspace.yaml`. The reverse direction is the one that has actually cost something here: a `--filter` naming a package the workspace does **not** define is a failure, not a shrug, because pnpm answers it by matching nothing and **exiting 0** — that is how `pnpm --filter app` let `nuxt build` silently never run and shipped an image built from a stale `.output` (the postmortem sits in `.gitlab-ci.yml`'s `turboops-build` header). `pnpm -r` and the set-valued filter forms (`api...`, `[origin/main]`, `!api`, a path) genuinely have no single target and are recorded as **skipped**, never passed quietly — in this template that is their normal state, since `projects/` stays empty until `lt fullstack init` fills it.

  Workspace discovery for all of this lives in one place, `scripts/lib/workspace-packages.mjs`, shared by `check.mjs`, `check-workspace-consistency.mjs` and `check-ci-consistency.mjs`. It used to be copied into all three and the copies had drifted apart; each then resolved nothing under a slightly different but perfectly valid `pnpm-workspace.yaml` and reported that every rule held. It also counts **symlinked** members, which `lt fullstack init --api-link` / `--frontend-link` create and which a plain `Dirent.isDirectory()` filter reports as absent — that alone had made every link-mode workspace look empty to all three guards at once.
