...
> Since lt CLI 1.42.0 (2026-07-31) `auditConfig` **is** hoisted into projects
> generated by `lt fullstack init`, so a suppression written here applies to every
> package in the generated workspace — not just the one that justified it. That
> makes a *stale* entry worse than no entry, because it silently covers advisories
> nobody assessed. Delete an entry as soon as its advisory is fixed or its upstream
> range narrowed, and re-verify by stripping the block and re-running `pnpm audit`.
